...
def main() -> None:
    ...
    include_sample = (
        "{{ cookiecutter.include_sample_dbt_project }}".strip().lower() in {"1", "true", "yes"}
    )
    include_docker = "{{ cookiecutter.include_docker }}".strip().lower() in {"1", "true", "yes"}
